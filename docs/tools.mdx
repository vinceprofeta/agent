...
and re-generating if you pass `stopWhen: isStepCount(num)` where `num > 1` to
...
export const ideaSearch = createTool({
  ...
  inputSchema: z.object({ query: z.string().describe("The query to search for") }),
  execute: async (ctx, input, options): Promise<Array<Idea>> => {
...
In both cases, the input and options match the underlying AI SDK's `tool`
function.

You can also pass ordinary AI SDK tools directly, including `tool`,
`dynamicTool`, and provider-defined or provider-executed tools such as web
search, code execution, computer use, or MCP tools exposed by a provider. Use
`createTool` only when you want Convex to inject `ctx`, `userId`, `threadId`,
and `messageId` into a server-executed tool.
...
const llmTool = createTool({
  ...
  execute: async (ctx, input, options): Promise<string> => {
    const result = await generateText({
      ...
      prompt: [...options.messages, { role: "user", content: input.message }],
