...
For AI SDK 7, `@convex-dev/agent` translates this Convex-specific
`createTool` option into the SDK's call-level `toolApproval` configuration.
If you pass plain AI SDK tools directly, configure approval with AI SDK 7's
`toolApproval` option on the generation call.
